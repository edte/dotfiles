...
source.get_completions = function(self, params, callback)
	local client = self:_get_client()
	if client == nil then
		log.error(0)
		return callback({ is_incomplete_forward = false })
	end
	local trigger_characters = {}
	...
	local cursor_before_line = string.sub(params.line, 1, params.cursor[2])
	local trigger_character = nil
	for _, c in ipairs(trigger_characters) do
		local s, e = string.find(cursor_before_line, "(" .. vim.pesc(c) .. ")%s*$")
		if s and e then
			trigger_character = string.sub(cursor_before_line, s, s)
			break
		end
	end
	...
	log.error(self:_items(self.signature_help))
end
