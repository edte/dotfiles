...
- 只要回复里需要给出 mermaid / mermiad / 流程图 / 时序图 / 架构图，默认优先触发 `mermaid-kitty` skill，输出 ASCII 渲染结果，不要等我额外提醒。
- 改动或者解释前，最好看看所有代码，不能偷懒。
- 改动前，要做最小化修改（改动范围最小），但不排除必要的接口优化或跨模块调整
- 改动后，验证关键场景：正常情况、边界情况、错误情况各至少一个
- 给出的mermaid图，必须自检语法，可以被渲染，在暗黑主题上清晰可见
- 给出的mermaid图，必须要可以被暗黑主题渲染清晰
- 修改代码之后，要检查一下是否有报错,如果能编译的话要检查一下是否有报错
...
8. **回归测试 (Regression Test):** 验证修复不引入新问题，检查关键场景是否仍正常工作。
